...
### 2026-07-19（预设像素标注与实际出图不一致 → fail-open 裁切/缩放后处理）
- **症状**：UI 预设声明 `1080×1440`、`1080×1350`、`1200×1500` 等精确尺寸，但 Gemini 只接收比例 + 2K 档，GPT 又把 `3:4` 统一映射成 `1024×1536` (2:3)，导致比例和像素都不如实。
- **根因**：`lib/models.ts` 的 `width/height` 仅作展示，生成链路只把粗粒度 `aspectRatio` 传给上游，成功后没有任何像素级规范化。
- **修法**：新增 Sharp `normalizeGeneratedImage`，仅当比例偏差 >2% 时居中裁切，损失 >25% 则原样返回；裁后仅缩小或 ≤1.35× 放大到声明像素，任何错误 fail-open。产品/场景/sceneGroup 三条成功路径在原资金 inner try 内接线，肖像卡不变。
- **验证**：15 预设矩阵、非法 base64、非正声明尺寸全通过；Gemini 与 GPT low 真实 SSE 探针均得到 `1080×1440` PNG；`tsc/build/lint` 通过，lint 仅保留 12 条既有 `<img>` warning。
