...
export const ACTIVE_BACKEND: 'gemini' | 'openai' =
...
export async function generateImage(input: BackendInput): Promise<BackendResult> {
  if (!API_KEY) {
    return { success: false, error: 'API Key 未配置', backend: ACTIVE_BACKEND };
  }
  return ACTIVE_BACKEND === 'openai'
