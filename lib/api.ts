...
export function buildSceneGroupPrompt(options: SceneGroupGenerateOptions): string {
  ...
  const groupModelConsistencyRule = identityMode === 'follow_scene'
    ? 'the identity baseline for each output is the person in its own scene-base image. Keep the scene-base person\'s skin tone exactly (same tan depth and undertone — never paler or pinker), hair color/length/styling unchanged, body build unchanged, and age feeling unchanged; and apply a REQUIRED partial change to at least two visible facial features (eyes/nose/lips/jawline) so the output is a similar-type but clearly not identical person.'
    : hasAnchor
  ...
  const groupContinuityRule = identityMode === 'follow_scene'
    ? 'treat outputs as one set. Keep output size/framing logic, lighting, product color, fabric texture, silhouette proportions, and photographic language continuous across the group; model identity follows each image\'s own scene-base person with only partial facial feature adjustment.'
  ...
  const freezeIdentityRule = identityMode === 'follow_scene'
    ? '- Preserve the base person\'s expression, mood, makeup language, and hair styling; apply the REQUIRED partial facial-feature change (at least two of eyes/nose/lips/jawline) while keeping the base person\'s exact hair color, visible length category, texture, tucked-or-loose state, overall hairstyle, and skin tone unchanged.'
