...
export function buildSceneGroupPrompt(options: SceneGroupGenerateOptions): string {
  ...
  const groupModelConsistencyRule = identityMode === 'follow_scene'
    ? 'cast a different model of the same look for each output: keep the scene-base person\'s exact skin tone (same tan depth and undertone — never paler or pinker), hair color/length/styling, body build, and age feeling, but give her a clearly different face — different enough that face recognition would not match the scene-base person. Same vibe, different woman.'
    : hasAnchor
  ...
  const freezeIdentityRule = identityMode === 'follow_scene'
    ? '- Preserve the base person\'s expression, mood, makeup language, and hair styling, and keep her exact hair color, visible length category, texture, tucked-or-loose state, overall hairstyle, and skin tone unchanged — but the FACE belongs to a different model: visibly different nose, lips, eyes, and jawline, distinct enough that face recognition would not match the scene-base person.'
