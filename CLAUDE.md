...
`.claude/rules/*.md` **不会被自动注入上下文**（Claude Code 2.1.263 实测：`claude -p` 里问「加载了哪些 rules 文件」，回答是能列出文件名但它们不在上下文里）。自动加载的只有本文件。所以流程是：**读本文件 → 按上表定板块 → 手动打开那一份规则 → 只读它列出的文件**。这正是省 token 的地方，别为了「保险」把 12 份规则全读进来。

每份规则顶部的 `paths:` 是给人和 AI 看的**范围提示**，标明这个板块管哪些路径，不是自动加载开关。
