...
// Pin this approved verification to the two upstreams named in the 09-04 decision.
// Set before loading image-backends.ts because its environment configuration is module-scoped.
...
async function buildContact() {
  const panels = await Promise.all([
    ...
    contactPanel(ANCHOR_PATH, 'derived anchor · optimized prompt'),
    contactPanel(LOOK_PATH, 'follow_scene · optimized prompt'),
  ]);
...
const results = {
  experiment: '09-04 follow_scene prompt-deduplication verification',
  instruction: 'Do not score or judge identity similarity; contact sheet is for human review.',
  startedAt,
  completedAt: null,
